...
from app.api.application_attention import router as application_attention_router
...
app = FastAPI(title="INFIOS Application Support Workbench", version=VERSION, description="Local-first Application Support workbench for incident evidence, escalation notes, and RCA drafts.")
...
def health() -> dict[str, str]:
    return {"status": "ok", "service": "infios-app-support-workbench", "version": VERSION}
...
def markdown_report(incident: IncidentInput) -> dict[str, str]:
    ...
    return {"incident_id": incident.incident_id, "markdown": render_markdown_report(incident, analysis)}
...
def markdown_report_with_history(incident: IncidentInput) -> dict[str, str]:
    ...
    history_path = save_run_history(incident, analysis, source_path="api:/api/report/markdown/save", output_format="markdown")
    return {"incident_id": incident.incident_id, "markdown": markdown, "history_path": str(history_path)}

def load_sample(name: str = "incident-500-login.json") -> IncidentInput:
    sample_path = SAMPLES_DIR / name
    return IncidentInput.model_validate(json.loads(sample_path.read_text(encoding="utf-8")))