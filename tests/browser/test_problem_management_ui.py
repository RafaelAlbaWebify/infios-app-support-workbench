...
def test_problem_detail_audit_known_error_and_action_filters(page: Page, problems_base_url: str) -> None:
    ...
    page.get_by_label("Search actions").fill("queue")
    ...
    page.get_by_label("Search actions").fill("")
    page.get_by_label("Status", exact=True).nth(1).select_option("planned")
    expect(page.locator("#action-count")).to_have_text("1 of 2 actions")
    expect(page.get_by_text("Review action plan")).to_be_visible()

    page.get_by_label("Status", exact=True).nth(1).select_option("")
    page.get_by_label("Type", exact=True).select_option("monitoring")
    page.get_by_label("Owner", exact=True).nth(1).fill("L2")
    expect(page.locator("#action-count")).to_have_text("1 of 2 actions")
    expect(page.get_by_text("Monitor order queue")).to_be_visible()

    page.get_by_label("Owner", exact=True).nth(1).fill("missing")
