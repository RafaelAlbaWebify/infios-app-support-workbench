...
def test_handover_route_serves_page() -> None:
    response = TestClient(app).get("/handovers")
    assert response.status_code == 200
    assert "Shift handovers" in response.text
    assert "immutable operator-authored snapshots" in response.text
...
def test_handover_list_and_detail(page: Page, handover_base_url: str) -> None:
    handover = {
        ...
        "cases": [
            {
                "case_id": "case-100",
                "status_summary": "Evidence collection is complete.",
                "next_action": "Review the linked application logs.",
                "blocker": "Waiting for the application owner.",
                "attention_required": True,
            },
            {
                "case_id": "case-101",
                "status_summary": "User impact has stopped.",
                "next_action": "Confirm the monitoring result.",
                "blocker": None,
    ...
    def api(route: Route) -> None:
        ...
        if url.endswith("/api/handovers?limit=100"):
            route.fulfill(json={"handovers": [handover], "count": 1})
        elif url.endswith("/api/handovers/handover-1"):
